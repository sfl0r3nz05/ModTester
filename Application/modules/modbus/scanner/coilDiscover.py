...
class Module:
	...
	info = {
		'Name': 'Modbus Coil Discover',
		'Author': ['@Danel'],
		'Description': (" Modbus Coilstatus discover"),
	...
	def do(self,ip):
		...
		if (c != None):
			...
			while(addressInt <= top):
				...
				ans = c.sr1(ModbusADU(transId=getTransId(),unitId=int(self.options['UID'][0]))/ModbusPDU01_Read_Coils(startAddr=addressInt,quantity=1),timeout=timeout, verbose=0)
				ans = ModbusADU_Answer(str(ans))
				#self.printLine('[+] Response is :',bcolors.OKGREEN)
				#ans.show()
				
				#La respuesta es correcta
				if(ans.funcCode != 0x81):
					MEM[0].append(addressInt)
					MEM[1].append(ans.coilStatus)
			...
			print '-------------------------------------------'
			print 'Coils en memoria: {}'.format(len(MEM[0]))
