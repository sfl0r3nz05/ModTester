...
class Module:
	...
	info = {
		'Name': 'Modbus Discrete Input Discover',
		'Author': ['@Danel'],
		'Description': (" Modbus Discrete Input Scanner"),
	...
	def do(self,ip):
		...
		if (c != None):
			...
			while(addressInt <= top ):
				...
				ans = c.sr1(ModbusADU(transId=getTransId(),unitId=int(self.options['UID'][0]))/ModbusPDU02_Read_Discrete_Inputs(startAddr=addressInt,quantity=1),timeout=timeout, verbose=0)
				...
				if(ans.funcCode != 0x82):
					MEM[0].append(addressInt)
					MEM[1].append(ans.inputStatus)
			...
			print '-------------------------------------------'
			print 'Discrete Inputs en memoria: {}'.format(len(MEM[0]))
