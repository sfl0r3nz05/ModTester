...
class Module:
	...
	info = {
		'Name': 'Modbus Holding Register Discover',
		'Author': ['@Danel'],
		'Description': (" Modbus Holding Register Scanner"),
	...
	def do(self,ip):
		...
		if (c != None):
			...
			while(addressInt <= top ):
				...
				ans = c.sr1(ModbusADU(transId=getTransId(),unitId=int(self.options['UID'][0]))/ModbusPDU03_Read_Holding_Registers(startAddr=addressInt,quantity=1),timeout=timeout, verbose=0)
				ans = ModbusADU_Answer(str(ans))
				#self.printLine('[+] Response is :',bcolors.OKGREEN)
				#ans.show()
				#print "Holding Register : {} ".format(addressInt)
			...
			print '-------------------------------------------'
			print 'Holding Registers en memoria: {}'.format(len(MEM))
